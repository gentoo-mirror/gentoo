...
IUSE="doc examples exif gd jpeg nls serial tiff ${IUSE_CAMERAS[@]/#/+cameras_}"
...
RDEPEND="
	...
	tiff? ( >=media-libs/tiff-4.0[${MULTILIB_USEDEP}] )
"
DEPEND="${RDEPEND}"
BDEPEND="
...
src_prepare() {
	...
	# Don't append -Werror, bug #977207
	sed -i -e '/GP_CONDITIONAL_COMPILE_FLAGS.*\[-Werror/d' configure.ac || die
...
multilib_src_configure() {
	...
	econf \
		...
		udevscriptdir="${EPREFIX}$(get_udevdir)" \
		"${myconf[@]}"
}
...
pkg_postinst() {
	if ! has_version "sys-apps/systemd[acl]" && ! has_version "sys-auth/elogind[acl]" ; then
