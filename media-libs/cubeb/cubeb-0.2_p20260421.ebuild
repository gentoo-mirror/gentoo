...
HASH_CUBEB=a37dadd1ed5949ab0accd7087b7c5c57800eab43
...
PATCHES=(
	...
	"${FILESDIR}"/${PN}-0.2_p20260421-alsa-ub.patch
)

CMAKE_SKIP_TESTS=(
	# needs access to real audio devices, and without sandbox
	audio
	callback_ret
	devices
	latency
...
src_prepare() {
	...
	# these tests currently fail to build unless use static libs
	sed -i '/cubeb_add_test(\(audio_dump\|logging\))/d' CMakeLists.txt || die
}
